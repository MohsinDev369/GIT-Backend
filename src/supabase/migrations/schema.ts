import { pgTable, foreignKey, uuid, numeric, text, timestamp, check, doublePrecision, boolean, time, real, integer, date, smallint, pgEnum, pgPolicy } from "drizzle-orm/pg-core"
...
export const usersSelectPolicy = pgPolicy("users_select_policy", {
  for: "select",
  to: "authenticated",
  using: sql`auth.uid()::text = user_id::text`
}).link(users)

export const usersUpdatePolicy = pgPolicy("users_update_policy", {
  for: "update",
  to: "authenticated",
  using: sql`auth.uid()::text = user_id::text`
}).link(users)
...
export const reviewsInsertPolicy = pgPolicy("reviews_insert_policy", {
  for: "insert",
  to: "authenticated",
  withCheck: sql`auth.uid()::text = user_id::text`
}).link(reviews)

export const reviewsUpdatePolicy = pgPolicy("reviews_update_policy", {
  for: "update",
  to: "authenticated", 
  using: sql`auth.uid()::text = user_id::text`
}).link(reviews)

export const eventsSelectPolicy = pgPolicy("events_select_policy", {
  for: "select",
  to: "public",
  using: sql`true`
}).link(events)

export const reservationsSelectPolicy = pgPolicy("reservations_select_policy", {
  for: "select",
  to: "authenticated",
  using: sql`auth.uid()::text = user_id::text`
}).link(reservations)

export const reservationsInsertPolicy = pgPolicy("reservations_insert_policy", {
  for: "insert", 
  to: "authenticated",
  withCheck: sql`auth.uid()::text = user_id::text`
}).link(reservations)

export const tablesSelectPolicy = pgPolicy("tables_select_policy", {
  for: "select",
  to: "public",
  using: sql`true`
}).link(tables)

export const promotionsSelectPolicy = pgPolicy("promotions_select_policy", {
  for: "select",
  to: "public", 
  using: sql`true`
}).link(promotions)

export const tasteSelectPolicy = pgPolicy("taste_select_policy", {
  for: "select",
  to: "public",
  using: sql`true`
}).link(tasteOfTheDay)

export const filterUsageInsertPolicy = pgPolicy("filter_usage_insert_policy", {
  for: "insert",
  to: "public",
  withCheck: sql`true`
}).link(filterUsage)